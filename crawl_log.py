...
def reset_crawl_times():
    ...
    for ip in ipJson:
        ...
        if not row[7]:
            last_record = {'上週': 0, '昨天': 0, '前小時': 0}
        else:
            last_record = json.loads(row[7])

        if day_of_week == 0 and current_hour == 0 and current_minute == 0: #重置(週一0點)
        ...
        elif current_hour == 0 and current_minute == 0: #重置(每天0點)
            last_record['昨天'] = crawl_times_day
            last_record['前小時'] = crawl_times_hour
            sql = "`crawl_times_day`=0, `crawl_times_hour`=0, "

        elif current_minute == 0: #重置(整點)
            last_record['前小時'] = crawl_times_hour
